...
AddEventHandler('tpz_society:createNewBill', function(targetId, isJob, cost, reason, issuer)
    ...
    Citizen.CreateThread(function ()
        ...
        exports.ghmattimysql:execute("SELECT * FROM billing WHERE reason = @reason AND issuer = @issuer AND charidentifier = @charidentifier AND date = @date", Parameters, function(res)
            
            if res[1] then 
                Billing[tonumber(res[1].id)] = {} 
                Billing[tonumber(res[1].id)] = res 
            end
